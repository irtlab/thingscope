...
class SecurityAnalyzer:
    def __init__(self, device_mac_address="", device_title="", sink=None, sink_interval=10, is_save_pcap=False, iface_mac_addr=None, ignore_mac_addrs="", internal_ip_prefix=""):
        ...
        self.devices = set()
        self.endpoint_security_prefixes = ['SSL connection using', 'subject:', 'start date:', 'expire date:',
                                  'subjectAltName:', 'issuer:']
    ...
    def do_endpoint_security(self, endpoint):
        print(f'Starting Endpoint Security On {endpoint}')
        debug_entries = []
        self_signed = False

        def debug_callback(t, b):
            #print(f'debug_callback {t} {b}')
            debug_entries.append((t, b))
        ...
        def is_security_related(msg):
            for pfx in self.endpoint_security_prefixes:
                if pfx in msg:
                    return True
            return False
        ...
        try:
            b_obj = BytesIO()
            crl = pycurl.Curl()
            crl.setopt(crl.URL, endpoint)
            crl.setopt(pycurl.VERBOSE, 1)
            crl.setopt(pycurl.SSL_VERIFYPEER, 0)
            crl.setopt(pycurl.CONNECTTIMEOUT, 5)
            crl.setopt(pycurl.TIMEOUT, 3)
            crl.setopt(pycurl.DEBUGFUNCTION, debug_callback)
        ...
        for dbg in debug_entries:
            dmsg = str(dbg[1])
            if (dmsg.__contains__('self signed certificate')):
                self_signed = True
                security_messages.append('Self Signed Certificate')
            if (is_security_related(dmsg)):
                security_messages.append(clean_msg(dmsg))
    ...
    def identify_tls_payload_type(self, pkt):
        try:
            ...
            print(f'post processing exception for {e}' + str(e))
            return None
    ...
    #
    # parse the Cipher Suite to extract key_exchange, auth_algo, encryption_algo
    # operation_auth_mode & hashing_algo
    #
    def get_cert_props(self, smsgs):
        cipher_info = {}
        for msg in smsgs:
            if msg.startswith('SSL connection using'):
                msgs = msg.split(' ')
                cipher_suit = msgs[len(msgs) - 1]

                cipher_info['openssl_name'] = cipher_suit

                try:
                    iana_name = self.openssl_iana_map.get(cipher_suit)

                    if iana_name:
                        response = requests.get(f'https://ciphersuite.info/api/cs/{iana_name}/').json()
                        if response.get(iana_name):
                            cs_info = response[iana_name]
                            tls_version = cs_info.get('tls_version')
                            if tls_version:
                                tls_version = ','.join(tls_version)
                            cs_info['cert_tls_version'] = tls_version
                            cs_info.pop('tls_version')
                            return cs_info

                except Exception as e:
                    print(f'exception parsing openssl name {cipher_suit} {e}')

                return cipher_info
        return cipher_info

    def is_self_signed_cert(self, smsgs):
        for msg in smsgs:
            if msg.__contains__('Self Signed Certificate'):
                return True
        return False
    ...
    def get_location(self, ip_address):
        ...
        c.setopt(c.URL, f'https://ipapi.co/{ip_address}/json/')
    ...
    def postProcess(self):
        ...
        if not df.empty:
            for index, group in df.groupby('ip'):
                ...
                print(f'index {index } group {domains}')
                ip_domain_map[index] = max(domains, key= len)

        print(f'ip_domain_map {ip_domain_map}')
        print(f'domain_cnames {self.domain_cnames}')
                ...
                try:
                    ...
                    if ip in self.ip_tls_map:
                        ...
                        try:
                            if ip in ip_domain_map:
                                domain_name = ip_domain_map[ip]
                                smsgs = security_analyzer.do_endpoint_security('https://' + str(domain_name))
                                endpoints_map[ip]['self_signed'] = self.is_self_signed_cert(smsgs)
                                self.update_cipher_info(ip, endpoints_map, self.get_cert_props(smsgs))
                                endpoints_map[ip]['cert_details'] = smsgs
                        except Exception as e:
                            print(f'post processing exception for {ip} {e}')

                    if not str(ip).startswith(self.internal_ip_prefix):
                        location_data = self.get_location(ip)
                        print(f'{ip} location {location_data}')
    ...
    def printStore(self):
        print(self.device_endpoints_store)
    ...
    def pktq_consumer(self):
        ...
        print('Starting pkt q consumer')
        while True:
            pkt = self.pktq.get()

            if ( time.time() - time_since_last_log > 5):
                print(f'pktq size {self.pktq.qsize()} {pkt.summary()} total recvd pkts {self.recvd_pkts_count} total processed pkts {self.processed_pkts_count}')
                time_since_last_log = time.time()
    ...
    def dns_callbackV2(self, pkt):
        ...
        def process_dns_packet(packet):
            ...
            if packet.haslayer(scapy.DNSRR) and packet[scapy.DNSRR].type == 1:  # 1 is stands for 'A' DNS record
                ...
                print('****[DNS-A]\t' + str(dest_ip) + ' <<< ' + str(domain_name) + ' (' + str(domain_ip_address) + ')')
            if packet.haslayer(scapy.DNSRR) and packet[scapy.DNSRR].type == 5:  # 1 is stands for 'CNAME' DNS record
                ...
                print('****[DNS-CNAME]\t' + str(dest_ip) + ' <<< ' + domain_name + ' (' + cname + ')')
...
if __name__ == "__main__":
    ...
    parser.add_argument("--internal_ip_prefix", help="Internal ip prefix", default="10.",
                        required=False)
