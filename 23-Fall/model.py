...
def cnn(x_train,x_test,features,dict_weights):
    ...
    model.add(Reshape((len(features),1)))
    model.add(Conv1D(256, kernel_size=2,strides=1, activation='relu', input_shape=(len(features),)))
    model.add(MaxPooling1D(pool_size=2, strides=1))
    model.add(Conv1D(256, kernel_size=2,strides=1, activation='relu'))
    model.add(MaxPool1D(pool_size=2, strides=1))
    model.add(Conv1D(256, kernel_size=2, strides=1, activation='relu'))
    model.add(Flatten())
    model.add(Dense(31, activation='softmax'))
    adam=K.optimizers.Adam(learning_rate=5e-2)
    model.compile(loss='categorical_crossentropy', optimizer=adam, metrics=['accuracy'])
    history=model.fit(x=X_train,y=y_train,validation_data= [X_test,y_test], epochs=300,batch_size=1024,
                      class_weight=dict_weights,
                      steps_per_epoch=X_train.shape[0]//1024,
                      validation_steps=X_test.shape[0]//1024)
    model.summary()
...
def xgboost(x_train,x_test,features,target,dict_weights):
    ...
    booster = XGBoostModel(features=features, target=target, num_boost_round=100, params=params)
...
def adaboost(x_train,x_test,features,target,dict_weights):
    x_train = vx.from_pandas(df=x_train, copy_index=True)
    x_test = vx.from_pandas(df=x_test, copy_index=True)
    clf = tree.DecisionTreeClassifier(max_depth=100,class_weight=dict_weights)
    clf = AdaBoostClassifier(estimator= clf,n_estimators=500, random_state=0,learning_rate=0.01)
