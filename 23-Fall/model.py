...
def random_forest(x_train,x_test,features,target,max_depth):
    x_train = vx.from_pandas(df=x_train, copy_index=True)
    x_test = vx.from_pandas(df=x_test, copy_index=True)
    clf = RandomForestClassifier(max_depth=max_depth, random_state=0, class_weight='balanced')
...
def xgboost(x_train,x_test,features,target):
    x_train = vx.from_pandas(df=x_train, copy_index=True)
    x_test = vx.from_pandas(df=x_test, copy_index=True)
...
def catboost(x_train,x_test,features,target):
    x_train = vx.from_pandas(df=x_train, copy_index=True)
    x_test = vx.from_pandas(df=x_test, copy_index=True)
    params = {
...
def adaboost(x_train,x_test,features,target):
    x_train = vx.from_pandas(df=x_train, copy_index=True)
    x_test = vx.from_pandas(df=x_test, copy_index=True)
    estimator=RandomForestClassifier(max_depth=10, random_state=0, class_weight='balanced')
    clf =  AdaBoostClassifier(estimator= estimator,n_estimators=50, random_state=0,learning_rate=1e-4)
    vaex_model = Predictor(features=features, target=target, model=clf, prediction_name='pred')
    vaex_model.fit(df=x_train)
    x_test = vaex_model.transform(x_test)
    return x_train, x_test